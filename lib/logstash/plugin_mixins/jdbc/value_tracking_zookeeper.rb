  ...
  class NodeHandler
    ...
    def create_node
      unless @exists
        if @zk_ephemeral
          traversed_path = ""
          @path.split('/').reject {|c| c.empty?}.each {|p|
            unless @zk.exists?("#{traversed_path}/#{p}")
              @zk.create("#{traversed_path}/#{p}", :ephemeral => true)
            end
            traversed_path = "#{traversed_path}/#{p}"
          }
        else
          traversed_path = ""
          @path.split('/').reject {|c| c.empty?}.each {|p|
            unless @zk.exists?("#{traversed_path}/#{p}")
              @zk.create("#{traversed_path}/#{p}")
            end
            traversed_path = "#{traversed_path}/#{p}"
          }
        end
